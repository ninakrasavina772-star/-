...
onnxruntime подгружается только в режиме neural — приложение в Streamlit Cloud стартует даже при проблемах с ONNX.
"""
...
# Максимальная сторона входа для neural (облако часто падает по RAM на больших x4)
_NEURAL_MAX_SIDE = int(os.environ.get("NEURAL_MAX_INPUT_SIDE", "512"))
...
def process_pil(
    ...
    *,
    ...
    w, h = im.size
    m = max(w, h)
    if _NEURAL_MAX_SIDE > 0 and m > _NEURAL_MAX_SIDE:
        s = _NEURAL_MAX_SIDE / m
